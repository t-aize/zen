...
const log = createLogger("event:messageReactionRemove");
...
defineEvent({
	...
	execute: async (reaction, user) => {
		...
		log.debug(
			...
			`Reaction removed by ${user.tag ?? user.id}`,
		);
		...
		await logChannel.send({
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(
						`${userMention(user.id)} removed a reaction from a message in ${channelMention(reaction.message.channelId)}.`,
					)
