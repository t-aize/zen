...
const log = createLogger("event:stageInstanceDelete");
...
defineEvent({
	...
	execute: async (stageInstance) => {
		...
		log.debug(
			{ guildId: stageInstance.guild.id, stageId: stageInstance.id },
			`Stage instance deleted: ${stageInstance.topic}`,
		);

		const executor = await stageInstance.guild
			.fetchAuditLogs({ type: AuditLogEvent.StageInstanceDelete, limit: 1 })
		...
		await logChannel.send({
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(`The stage session in ${channelMention(stageInstance.channelId)} has ended.`)
					.setColor(Colors.Red)
					.addFields(
						{
							name: "📋 Details",
