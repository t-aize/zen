...
import { ensureGuild, executorFieldWithDuration } from "@/utils/moderation.js";
...
defineCommand({
	...
	execute: async (interaction) => {
		...
		const oldSlowmode = "rateLimitPerUser" in targetChannel ? (targetChannel.rateLimitPerUser as number) : 0;
		const startedAt = new Date();

		try {
			await targetChannel.setRateLimitPerUser(seconds, `[Zen] Set by ${interaction.user.tag}`);
		} catch (err) {
			log.error({ err, channelId: targetChannel.id }, "Failed to set slowmode");
			await interaction.editReply({
				content: blockQuote(`❌ ${bold("Failed")} — An unexpected error occurred while setting slowmode.`),
			});
			return;
		}

		log.info(
			{ channelId: targetChannel.id, executorId: interaction.user.id, oldSlowmode, newSlowmode: seconds },
		...
		await interaction.editReply({
			embeds: [
				new EmbedBuilder()
					.setTitle(seconds > 0 ? "🐌 Slowmode Updated" : "🐌 Slowmode Disabled")
					.setDescription(
						seconds > 0
