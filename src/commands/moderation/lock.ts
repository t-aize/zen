	...
	execute: async (interaction) => {
		...
		try {
			await targetChannel.permissionOverwrites.edit(
				...
				{ reason: `[Zen] Locked by ${interaction.user.tag}: ${reason}`, type: OverwriteType.Role },
			);
		} catch (err) {
			log.error({ err, channelId: targetChannel.id }, "Failed to lock channel");
			await interaction.editReply({
				content: blockQuote(`❌ ${bold("Failed")} — An unexpected error occurred while locking the channel.`),
			});
			return;
		}
		...
		await interaction.editReply({
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(`${channelMention(targetChannel.id)} has been locked.`)
