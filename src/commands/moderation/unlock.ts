...
import { ensureGuild, executorFieldWithDuration, reasonField } from "@/utils/moderation.js";
...
defineCommand({
	data: new SlashCommandBuilder()
		.setName("unlock")
		.setDescription("Unlock a channel — restore @everyone's ability to send messages.")
	...
	execute: async (interaction) => {
		...
		try {
			await targetChannel.permissionOverwrites.edit(
				...
				{ reason: `[Zen] Unlocked by ${interaction.user.tag}: ${reason}`, type: OverwriteType.Role },
			);
		} catch (err) {
			log.error({ err, channelId: targetChannel.id }, "Failed to unlock channel");
			await interaction.editReply({
				content: blockQuote(`❌ ${bold("Failed")} — An unexpected error occurred while unlocking the channel.`),
			});
			return;
		}
		...
		await interaction.editReply({
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(`${channelMention(targetChannel.id)} has been unlocked.`)
