...
export const buildTimedOutEmbed = (action: string) =>
	new EmbedBuilder()
		...
		.setDescription(
			`The confirmation prompt expired after 30 seconds. ${action}`,
		)
		.setColor(Colors.Grey)
		.setFooter({ text: "Zen • Moderation" })
		.setTimestamp();
...
export const targetFieldRaw = (
	...
	value: blockQuote(
		[
			`${inlineCode("User:")} ${bold(tag)}`,
			`${inlineCode("ID:")}   ${inlineCode(id)}`,
			...(extra ?? []),
		].join("\n"),
	),
	inline,
 ...
 */
export const executorFieldWithDuration = (
	executor: { tag: string },
	startedAt: Date,
	inline = true,
) => ({
...
export const ensureGuild = async (
	...
	await interaction.reply({
		content: blockQuote(
			`⛔ ${bold("Server only")} — This command cannot be used in DMs.`,
		),
 ...
 */
export const replyMemberNotFound = async (
	interaction: ChatInputCommandInteraction,
	userId: string,
) => {
 ...
 */
export const replyActionBlocked = async (
	interaction: ChatInputCommandInteraction,
	reason: string,
) => {
	await interaction.editReply({
 ...
 */
export const pluralise = (n: number, word: string) =>
	`${bold(String(n))} ${word}${n !== 1 ? "s" : ""}`;
