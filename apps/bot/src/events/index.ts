 ...
 * });
 */
export const defineEvent = <K extends keyof ClientEvents>(
	event: Event<K>,
): Event<K> => {
	...
	const duplicate = events.find(
		(e) => e.name === event.name && e.once === event.once,
	);
	if (duplicate) {
		log.warn(
			`defineEvent: duplicate handler registered for "${event.name}" (once: ${event.once ?? false})`,
		);
	}
