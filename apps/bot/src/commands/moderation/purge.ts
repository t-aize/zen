...
defineCommand({
	data: new SlashCommandBuilder()
		.setName("purge")
		.setDescription(
			"Delete all recent messages from a specific user across the entire server.",
		)
		...
		.addUserOption((opt) =>
			opt
				.setName("user")
				.setDescription("The user whose messages will be purged.")
				.setRequired(true),
		)
		.addIntegerOption((opt) =>
			opt
				.setName("limit")
				.setDescription(
					"Max messages to delete per channel (1–100). Defaults to 100.",
				)
				.setMinValue(1)
	...
	execute: async (interaction) => {
		...
		await interaction.editReply({
			...
			components: [
				buildConfirmRow(CONFIRM_ID, CANCEL_ID, "Confirm Purge", "🗑️"),
			],
		});
		...
		createConfirmationCollector({
			...
			onConfirm: async () => {
				...
				for (const channel of textChannels.values()) {
					try {
						const [toDelete, skipped] = await collectFromChannel(
							channel,
							targetUser.id,
							limit,
						);
