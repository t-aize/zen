...
const getNicknameBlockReason = (
	executor: GuildMember,
	target: GuildMember,
	me: GuildMember,
): string | null => {
	if (!target.manageable)
		return "I don't have permission to manage this member.";
	if (target.id === me.id)
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((opt) =>
			opt
				.setName("nickname")
				.setDescription(
					"The new nickname. Leave empty to reset to their username.",
				)
				.setMinLength(1)
				.setMaxLength(32)
				.setRequired(false),
	...
	execute: async (interaction) => {
		...
		try {
			await target.setNickname(
				newNick,
				`[Zen] Changed by ${executor.user.tag}`,
			);
		} catch (err) {
			log.error(
			...
			await interaction.editReply({
				content: blockQuote(
					`❌ ${bold("Failed")} — An unexpected error occurred while changing the nickname.`,
				),
			});
			return;
		}
