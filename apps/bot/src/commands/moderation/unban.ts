...
const buildResultEmbed = (
	...
	new EmbedBuilder()
		...
		.setDescription(
			`${userMention(targetId)} has been successfully unbanned from the server.`,
		)
		...
		createConfirmationCollector({
			...
			onConfirm: async () => {
				...
				try {
					...
					log.error(
						{ err, targetId, executorId: executor.id },
				...
				log.info(
					{ targetId, executorId: executor.id, reason },
					`${executor.user.tag} unbanned ${targetTag}`,
				);
