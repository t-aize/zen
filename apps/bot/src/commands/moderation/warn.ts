...
import { warnings } from "@zen/db/schema";
import {
	blockQuote,
	bold,
	Colors,
	...
	execute: async (interaction) => {
			...
			createConfirmationCollector({
				...
				onConfirm: async () => {
					...
					try {
						...
						if (!inserted) {
							log.error(
								{ targetId: target.id, executorId: executor.id },
						...
						const all = await db.query.warnings.findMany({
							where: (w, { eq, and }) =>
								and(
									eq(w.guildId, interaction.guildId),
									eq(w.userId, target.id),
								),
						});
					...
					try {
						await target.send({
							embeds: [
								new EmbedBuilder()
									...
									.setDescription(
										`You received a warning in ${bold(interaction.guild.name)}.`,
									)
									.setColor(Colors.Yellow)
						...
						log.debug(
							{ targetId: target.id },
							"Could not DM warning to member (DMs likely closed)",
						);
					}
		...
		if (sub === "list") {
			...
			try {
				...
				await interaction.editReply({
					content: blockQuote(
						`❌ ${bold("Failed")} — Could not fetch warnings from the database.`,
					),
				});
				return;
			}
			...
			const pageWarnings = all.slice(
				(safePage - 1) * WARNINGS_PER_PAGE,
			...
			if (!hasWarnings) {
				embed.setDescription(
					`${userMention(targetUser.id)} has a ${bold("clean record")} on this server. ✅`,
				);
			} else {
				embed.setDescription(
				...
				embed.addFields(
					...
					...pageWarnings.map((w, i) => {
						...
						return {
							name: `${bold(`#${num}`)} · ${time(w.createdAt, TimestampStyles.ShortDate)} · ${inlineCode(String(w.id))}`,
							value: blockQuote(
								[
		...
		if (sub === "remove") {
			...
			try {
				warn = await db.query.warnings.findFirst({
					where: (w, { eq, and }) =>
						and(eq(w.id, warnId), eq(w.guildId, interaction.guildId)),
				});
			} catch (err) {
				log.error({ err, warnId }, "Failed to fetch warning for removal");
				await interaction.editReply({
					content: blockQuote(
						`❌ ${bold("Failed")} — Could not fetch the warning from the database.`,
					),
				});
				return;
			}
		...
		if (sub === "clear") {
			...
			try {
				...
				log.error(
					{ err, targetId: targetUser.id },
					"Failed to fetch warnings for clear",
				);
				await interaction.editReply({
					content: blockQuote(
						`❌ ${bold("Failed")} — Could not fetch warnings from the database.`,
			...
			createConfirmationCollector({
				...
				onConfirm: async () => {
					...
					try {
						...
						await interaction.editReply({
							content: blockQuote(
								`❌ ${bold("Failed")} — Could not clear warnings from the database.`,
							),
							components: [],
						});
						return;
