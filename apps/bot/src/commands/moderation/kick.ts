...
const buildResultEmbed = (
	...
	new EmbedBuilder()
		...
		.setDescription(
			`${userMention(target.id)} has been successfully kicked from the server.`,
		)
		.setThumbnail(target.displayAvatarURL())
...
defineCommand({
	...
	execute: async (interaction) => {
		...
		await interaction.editReply({
			...
			components: [
				buildConfirmRow(CONFIRM_ID, CANCEL_ID, "Confirm Kick", "👢"),
			],
		});
				...
				try {
					...
					await interaction.editReply({
						embeds: [
							buildErrorEmbed(
								"Kick Failed",
								"An unexpected error occurred while kicking the member.",
							),
						],
						components: [],
					});
