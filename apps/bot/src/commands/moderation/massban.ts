	...
	data: new SlashCommandBuilder()
		...
		.addIntegerOption((opt) =>
			opt
				...
				.setDescription(
					"Delete messages from the last N days (0–7). Defaults to 1.",
				)
				.setMinValue(0)
				.setMaxValue(7)
				.setRequired(false),
	...
	execute: async (interaction) => {
		...
		if (ids.length === 0) {
			await interaction.editReply({
				content: blockQuote(
					`⛔ ${bold("No valid IDs")} — Could not find any valid user IDs in your input.`,
				),
			});
			return;
		}
		...
		if (ids.length > 200) {
			await interaction.editReply({
				content: blockQuote(
					`⛔ ${bold("Too many IDs")} — Maximum is ${bold("200")} users per mass ban.`,
				),
			});
			return;
		}
		...
		const filtered = ids.filter(
			(id) => id !== interaction.user.id && id !== selfId,
		);

		if (filtered.length === 0) {
			await interaction.editReply({
				content: blockQuote(
					`⛔ ${bold("No valid targets")} — You cannot ban yourself or the bot.`,
				),
			});
			return;
		}
		...
		await interaction.editReply({
			...
			components: [
				buildConfirmRow(CONFIRM_ID, CANCEL_ID, "Confirm Mass Ban", "⚠️"),
			],
		});
			...
			onConfirm: async () => {
				...
				await interaction.editReply({
					embeds: [
						new EmbedBuilder()
							.setTitle(
								failed === 0 ? "✅ Mass Ban Complete" : "⚠️ Mass Ban Partial",
							)
							.setDescription(
								failed === 0
